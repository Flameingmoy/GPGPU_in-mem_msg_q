    ...
    with GpuQueue(cfg) as q:
        ...
        while (time.time() - start_time) < duration_s:
            ...
            if now - last_report >= report_interval:
                ...
                print(
                    f"  [{progress:5.1f}%] Rate: {rate:,.0f} msg/s | "
...
def main():
    ...
    parser.add_argument(
        "--track-a", "-a", action="store_true", help="Include Track A (Redis) soak test"
    )
    parser.add_argument(
        "--track-b-only", "-b", action="store_true", help="Only run Track B soak test"
    )
    parser.add_argument(
        "--report-interval",
        "-r",
    ...
    parser.add_argument("--csv", "-c", type=str, default=None, help="Save hardware metrics to CSV")
    ...
    try:
        ...
        if not args.track_a or not args.track_b_only:
            ...
            if initial_snap.gpus:
                ...
                stats_b.max_vram_gb = max(
                    s.gpus[0].vram_used_gb for s in monitor.get_snapshots() if s.gpus
                )
